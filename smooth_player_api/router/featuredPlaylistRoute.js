...
router.post(
  ...
  auth.verifyAdmin,
  featuredPlaylistUpload.single("featured_playlist_image"),
  function (req, res) {
    ...
    newFeaturedPlaylist.save().then(() => {
      res.status(201).send({
        resM: "'" + req.body.title + "'" + " featured playlist created.",
      });
    });
  }
);
...
router.delete(
  "/delete/featuredPlaylist",
  auth.verifyAdmin,
  async (req, res) => {
    const featuredSongs = await featuredSong.find({
      album: req.body.featuredPlaylistId,
    });
    const featuredPlaylistData = await featuredPlaylist.findOne({
      _id: req.body.featuredPlaylistId,
    });
    ...
    fs.unlinkSync(
      `../smooth_player_api/upload/image/featuredPlaylist/${featuredPlaylistData["featured_playlist_image"]}`
    );
